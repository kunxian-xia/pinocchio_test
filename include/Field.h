...
public:
	...
	void zero(FieldElt& elt) { memset(&(elt.data), 0, sizeof(FieldElt)); }
	static void Zero(FieldElt* elt) { memset(elt->data, 0, sizeof(FieldElt)); }
	//void zero(FieldElt* eltArray, int count) { memset(eltArray, 0, sizeof(FieldElt) * count); }

	// Set an existing field element to one
	void one(FieldElt& elt) { memcpy(&elt.data, msfield->one.data, sizeof(FieldElt)); }
